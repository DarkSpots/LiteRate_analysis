...
def get_prior_shift(t_start,t_end,bins_histogram, prior_reps):
    times_of_shift = []
    sampled_K = []
    # Gamma hyper-prior
...
def get_r_plot(res,col,parameter,min_age,max_age,plot_title,plot_log,run_simulation=1,BFs=None):
    ...
    # get BFs
    if run_simulation==1:
        BFs = get_prior_shift(min_age,max_age,bins_histogram,prior_reps)
        out_str += "\nbf2 = %s\nbf6 = %s" % (BFs[1],BFs[2])
    out_str += "\nabline(h=bf2, lty=2)"
    out_str += "\nabline(h=bf6, lty=2)"
...
def plot_net_diversity(div_log,resS,col,min_age,max_age,plot_title):
    ...
    out_str += "\nplot(time,net_diversity[(1+ length(net_diversity) - length(time)): length(net_diversity)],type = 'l', ylab = 'Net Diversity', xlab = 'Time (%s)',lwd=2, main='%s', col= '%s' )\n" \
...
def plot_marginal_rates(path_dir,name_tag="",bin_size=1.,burnin=0.2,min_age=0,max_age=0,logT=0,combine=0,ggplot=True, skipbins=0):
    ...
    for mcmc_file in files:
        try:
            ...
            if min_age==0 and max_age==0: # get empirical time range
                ...
                #if present_year == -1:
                max_age_t = np.mean(tbl[:,head.index("root_age")]) + skipbins
...
p.add_argument('-logT', metavar='0', type=int,help='set to 1 to log transform rates',default=0)
...
p.add_argument('-ggplot', metavar='1', type=int,help='set to 1 to plot using ggplot',default=1)
p.add_argument('-skipbins', metavar='0', type=int,help='skip first n. bins',default=0)
...
TBP=args.TBP
plot_marginal_rates(path_dir_log_files,logT=args.logT,burnin=args.burnin,combine=args.combine, ggplot=args.ggplot, skipbins=args.skipbins)
